                ...
                <div class="p-6">
                            ...
                            @if(session('va_number'))
                                <div class="bg-white p-4 rounded-lg border-2 border-indigo-500">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <p id="va-display" class="text-3xl font-bold text-indigo-600 tracking-wider">
                                            {{ session('va_number') }}
                                        </p>
                                    ...
                                    @if(session('va_amount'))
                                        <p class="text-sm text-gray-600 mt-3">Jumlah: <strong class="text-lg text-gray-800">Rp
                                                {{ number_format(session('va_amount'), 0, ',', '.') }}</strong></p>
                                    @endif
                                    <p class="text-xs text-gray-500 mt-2">Salin kode VA di atas dan masukkan pada form di bawah
                                    </p>
                                </div>
                            @endif
                        </div>
                    ...
                    <form action="{{ route('payment.check') }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="va_number" required placeholder="VA20241206..."
                                value="{{ request('va_number') ?? session('va_number') ?? old('va_number') }}"
                    ...
                    <div class="mt-6 p-4 bg-blue-50 rounded">
                        <p class="text-sm text-blue-800">
                            <strong>Info:</strong> Masukkan kode VA yang Anda dapatkan setelah checkout atau top up
                            wallet.
                        </p>
                    </div>
                </div>
